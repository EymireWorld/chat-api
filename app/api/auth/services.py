...
from app.api.auth.dependencies import encode_jwt, hash_password, validate_password
from app.api.auth.schemas import TokenSchema, UserSingInSchema, UserSingUpSchema
...
async def sing_up(session: AsyncSession, data: UserSingUpSchema):
    stmt = (
        insert(UserModel)
        .values(username=data.username, email=data.email, hashed_password=hash_password(data.password))
